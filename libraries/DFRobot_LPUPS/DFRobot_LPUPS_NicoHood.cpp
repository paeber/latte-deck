/*!
 * @file DFRobot_LPUPS_NicoHood.cpp
 * @brief  NicoHood-compatible version of DFRobot_LPUPS class
 * @n This is a Digital Metal-Oxide Multi-Gas Sensor. It can be controlled by I2C and SPI port.
 * @copyright Copyright (c) 2010 DFRobot Co.Ltd (http://www.dfrobot.com)
 * @license The MIT License (MIT)
 * @author [qsjhyy](yihuan.huang@dfrobot.com)
 * @version  V1.1 - NicoHood Compatible
 * @date  2023-12-01
 * @url https://github.com/DFRobot/DFRobot_LPUPS
 */
#include "DFRobot_LPUPS_NicoHood.h"

DFRobot_LPUPS_NicoHood::DFRobot_LPUPS_NicoHood()
{
}

int DFRobot_LPUPS_NicoHood::begin(uint16_t upsType)
{
  _upsType = upsType;
  uint8_t pidBuf[2];
...
void DFRobot_LPUPS_NicoHood::getChipData(uint8_t* regBuf)
{
  readReg(CS32_I2C_CHARGER_STATUS_REG, regBuf, CS32_I2C_SET_VBAT_LIMIT_REG + 2);
  delay(20);
}

void DFRobot_LPUPS_NicoHood::setMaxChargeVoltage(uint16_t data)
{
...
DFRobot_LPUPS_I2C_NicoHood::DFRobot_LPUPS_I2C_NicoHood(TwoWire* pWire, uint8_t i2cAddr)
{
  _deviceAddr = i2cAddr;
  _pWire = pWire;
}

int DFRobot_LPUPS_I2C_NicoHood::begin(uint16_t upsType)
{
  _pWire->begin();   // Wire.h(I2C)library function initialize wire library
  return DFRobot_LPUPS_NicoHood::begin(upsType);   // Use the initialization function of the parent class
}

void DFRobot_LPUPS_I2C_NicoHood::writeReg(uint8_t reg, const void* pBuf, size_t size)
{
  if (pBuf == NULL) {
...
size_t DFRobot_LPUPS_I2C_NicoHood::readReg(uint8_t reg, void* pBuf, size_t size)
{
  size_t count = 0;
  if (NULL == pBuf) {
    DBG("pBuf ERROR!! : null pointer");
